...
int blind_points_rejected = 0;
int raw_point_count = 0;
int front_points_rejected = 0;
int front_raw_point_count = 0;
int back_raw_point_count = 0;

namespace
{
template <typename LivoxMsgT>
void avia_handler_impl(
    const LivoxMsgT &msg,
    PointCloudXYZI &pl_surf,
    PointCloudXYZI &pl_corn,
    PointCloudXYZI &pl_full,
    PointCloudXYZI *pl_buff,
    std::vector<orgtype> *typess,
    ...
    Preprocess *self)
{
  ...
  if (feature_enabled)
  {
    for (uint i = 1; i < plsize; i++)
    {
      ...
      {
        const bool within_blind = (msg.points[i].x * msg.points[i].x + msg.points[i].y * msg.points[i].y +
                                   msg.points[i].z * msg.points[i].z) <= (blind * blind);
        if (within_blind)
        {
          blind_points_rejected++;
        }
    ...
    for (uint i = 1; i < plsize; i++)
    {
          ...
          ((msg.points[i].tag & 0x30) == 0x10 || (msg.points[i].tag & 0x30) == 0x00))
      {
        ...
        if (valid_num % point_filter_num == 0)
        {
          ...
          const bool within_blind = (pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y +
                                      pl_full[i].z * pl_full[i].z) <= (blind * blind);
          if (within_blind) blind_points_rejected++;
...
void Preprocess::process(const sensor_msgs::msg::PointCloud2::UniquePtr &msg, PointCloudXYZI::Ptr& pcl_out)
{
  raw_point_count = -1;
  blind_points_rejected = 0;
  front_points_rejected = 0;
  front_raw_point_count = 0;
  back_raw_point_count = 0;
  switch (time_unit)
  {
    case SEC:
...
{
  avia_handler_impl(*msg, pl_surf, pl_corn, pl_full, pl_buff, typess, feature_enabled, N_SCANS, point_filter_num, blind,
                    vx, vy, vz, this);
}
...
void Preprocess::mid360_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  const double scan_relative_time = rclcpp::Time(msg->header.stamp).seconds() - first_lidar_time;
  const bool front_reject_active = front_reject_enable &&
                                   scan_relative_time >= FRONT_REJECT_START_TIME &&
                                   scan_relative_time <= FRONT_REJECT_END_TIME;
  ...
  for (uint i = 0; i < plsize; ++i)
  {
    ...
    const bool front_reject_sector = added_pt.x >= 0.0;

    if (front_reject_sector) front_raw_point_count++;
    else back_raw_point_count++;
    ...
    if (front_reject_active && front_reject_sector)
    {
      front_points_rejected++;
      continue;
    }
