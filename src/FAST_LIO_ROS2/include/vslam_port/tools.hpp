...
#define VSLAM_G_m_s2 9.8
#define DIM 15
#define NMATCH 5

typedef pcl::PointXYZINormal PointType;
using namespace std;
namespace vslam_port {
...
} // namespace vslam_port (temporarily closed: std::hash specializations must be at global/std scope)
namespace std
{
  template<>
  struct hash<vslam_port::VOXEL_LOC>
  {
    size_t operator() (const vslam_port::VOXEL_LOC &s) const
    {
...
struct IMUST
{
  ...
  IMUST(double _t, const Eigen::Matrix3d &_R, const Eigen::Vector3d &_p, const Eigen::Vector3d &_v, const Eigen::Vector3d &_bg, const Eigen::Vector3d &_ba, const Eigen::Vector3d &_g = Eigen::Vector3d(0, 0, -VSLAM_G_m_s2)) : t(_t), R(_R), p(_p), v(_v), bg(_bg), ba(_ba), g(_g) {}
