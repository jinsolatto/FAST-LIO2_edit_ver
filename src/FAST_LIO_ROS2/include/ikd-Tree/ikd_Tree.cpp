...
void KD_TREE<PointType>::Search(KD_TREE_NODE *root, int k_nearest, PointType point, MANUAL_HEAP &q, float max_dist)
{
    ...
    if (!root->point_deleted)
    {
        ...
        // Non-strict (<=) so points exactly tied with the current worst kept
        // neighbor are still considered; PointType_CMP's x-coordinate
        // tie-break then decides the winner deterministically instead of
        // leaving it to traversal/tree-shape order.
