#!/bin/bash
# One-off run of bucheon05a.yaml with voxel_ramp_duration widened from 5s to
# 20s (same segments [110,140] and [305,425], voxel_size_normal=0.5,
# voxel_size_fine=0.1) to test whether a slower coarse<->fine transition
# reduces the trajectory drift observed at segment boundaries with the 5s
# ramp. Saves to BUCHEON/bucheon_05a/ramp20/0.1/.

set -e

REPO=/home/jschoi/edit_fastlio2
CONFIG=$REPO/src/FAST_LIO_ROS2/config/bucheon05a.yaml
OUTDIR=$REPO/BUCHEON/bucheon_05a/ramp20/0.1
BAG=/home/jschoi/bag_file/05_a_minimize
DIAG=$REPO/diag_test/ramp20
mkdir -p "$OUTDIR" "$DIAG"
...
echo "=== bucheon05a ramp20 (fine=0.1) : starting $(date) ==="

"$REPO/run_with_pidstat.sh" "bucheon05a_ramp20_01" bucheon05a.yaml > "/tmp/bucheon05a_ramp20_01_launch.log" 2>&1 &
LAUNCH_BG_PID=$!

until grep -q "준비 완료" "/tmp/bucheon05a_ramp20_01_launch.log" 2>/dev/null; do
    sleep 1
done
echo "[ramp20 0.1] node ready $(date)"

ros2 bag play "$BAG" --rate 1.0 --clock > "/tmp/bucheon05a_ramp20_01_bagplay.log" 2>&1
...
cp "$REPO/cpu_logs/cpu_log_bucheon05a_ramp20_01.txt" "$OUTDIR/cpu_log.txt"
echo "=== bucheon05a ramp20 (fine=0.1) : done $(date) ==="
