...
CONFIG=$REPO/src/FAST_LIO_ROS2/config/bucheon05d_flatten.yaml
OUTDIR=$REPO/BUCHEON/bucheon_05d_flatten
BAG=/home/jschoi/bag_file/05_d_minimize
DIAG=$REPO/diag_test/bucheon05d_flatten
mkdir -p "$OUTDIR" "$DIAG"
...
echo "=== bucheon05d_flatten : starting $(date) ==="

"$REPO/run_with_pidstat.sh" "bucheon05d_flatten" bucheon05d_flatten.yaml > "/tmp/bucheon05d_flatten_launch.log" 2>&1 &
LAUNCH_BG_PID=$!

until grep -q "준비 완료" "/tmp/bucheon05d_flatten_launch.log" 2>/dev/null; do
    sleep 1
done
echo "[flatten] node ready $(date)"

ros2 bag play "$BAG" --rate 1.0 --clock > "/tmp/bucheon05d_flatten_bagplay.log" 2>&1
echo "[flatten] bag playback finished $(date)"
...
if [ -n "$WIN_ID" ]; then
    ...
    echo "[flatten] map.png saved (window $WIN_ID)"
else
    echo "[flatten] WARNING: could not find rviz viewport window for screenshot"
fi
...
if [ -n "$LEFTOVER" ]; then
    echo "[flatten] WARNING: leftover processes still alive after cleanup:"
    echo "$LEFTOVER"
else
    echo "[flatten] confirmed: node/rviz/republish/pidstat all dead"
fi

cp "$REPO/cpu_logs/cpu_log_bucheon05d_flatten.txt" "$OUTDIR/cpu_log.txt"
echo "=== bucheon05d_flatten : done $(date) ==="
