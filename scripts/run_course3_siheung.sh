#!/bin/bash
# One-off: course3_minimize bag, siheung.yaml only (voxelslam-export
# matching, --rate 1.0), saved to
# turtleisland_260915/10degree/course3_minimize/siheung.yaml/
# (previous result there backed up to a _prev_<time> subfolder).

set -eo pipefail

REPO=/home/jschoi/edit_fastlio2
BAG=/home/jschoi/bag_file/10degree_/course3_minimize
OUTDIR="$REPO/turtleisland_260915/10degree/course3_minimize/siheung.yaml"
RUNNAME="course3_siheung_rerun"
...
echo "[course3/siheung.yaml] node ready"

ros2 bag play "$BAG" --rate 1.0 --clock > "/tmp/${RUNNAME}_bagplay.log" 2>&1
echo "[course3/siheung.yaml] bag playback finished"
...
if [ -n "$WIN_ID" ]; then
    ...
    echo "[course3/siheung.yaml] map.png saved (window $WIN_ID)"
else
    echo "[course3/siheung.yaml] WARNING: could not find rviz viewport window for screenshot"
fi

python3 "$REPO/scripts/plot_path_topdown.py" "$OUTDIR/trajectory.tum" "$OUTDIR/path.png"
echo "[course3/siheung.yaml] path.png saved"
...
if [ -n "$LEFTOVER" ]; then
    ...
    echo "[course3/siheung.yaml] confirmed: node/rviz/republish/pidstat all dead"
fi
