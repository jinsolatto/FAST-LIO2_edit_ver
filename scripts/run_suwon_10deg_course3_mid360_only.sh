#!/bin/bash
# bag_file/10degree/course3_minimize, mid360.yaml only (voxelslam-export
# matching, --rate 1.0, filter_size_surf=0.2), saved to
# suwon_260914/10degree/course3_minimize/mid360.yaml/voxel0.2/

set -eo pipefail

REPO=/home/jschoi/edit_fastlio2
BAG=/home/jschoi/bag_file/10degree/course3_minimize
OUTDIR="$REPO/suwon_260914/10degree/course3_minimize/mid360.yaml/voxel0.2"
RUNNAME="suwon_10deg_course3_mid360_voxel0.2_rerun"
...
echo "[course3/mid360.yaml/voxel0.2] node ready"

ros2 bag play "$BAG" --rate 1.0 --clock > "/tmp/${RUNNAME}_bagplay.log" 2>&1
echo "[course3/mid360.yaml/voxel0.2] bag playback finished"
...
if [ -n "$WIN_ID" ]; then
    ...
    echo "[course3/mid360.yaml/voxel0.2] map.png saved (window $WIN_ID)"
else
    echo "[course3/mid360.yaml/voxel0.2] WARNING: could not find rviz viewport window for screenshot"
fi

python3 "$REPO/scripts/plot_path_topdown.py" "$OUTDIR/trajectory.tum" "$OUTDIR/path.png"
echo "[course3/mid360.yaml/voxel0.2] path.png saved"
...
if [ -n "$LEFTOVER" ]; then
    ...
    echo "[course3/mid360.yaml/voxel0.2] confirmed: node/rviz/republish/pidstat all dead"
fi
