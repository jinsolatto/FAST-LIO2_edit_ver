...
# matching, --rate 1.5, filter_size_surf=0.2), saved to
# bucheon_260914/15degree/15_d/mid360.yaml/voxel0.2/

set -eo pipefail

REPO=/home/jschoi/edit_fastlio2
BAG=/home/jschoi/bag_file/15_d
OUTDIR="$REPO/bucheon_260914/15degree/15_d/mid360.yaml/voxel0.2"
RUNNAME="bucheon_15deg_15_d_mid360_voxel0.2"
...
if [ -n "$WIN_ID" ]; then
    ...
    echo "[15_d] map.png saved (window $WIN_ID)"
else
    echo "[15_d] WARNING: could not find rviz viewport window for screenshot"
fi

python3 "$REPO/scripts/plot_path_topdown.py" "$OUTDIR/trajectory.tum" "$OUTDIR/path.png"
echo "[15_d] path.png saved"
...
if [ -n "$LEFTOVER" ]; then
    ...
    echo "[15_d] confirmed: node/rviz/republish/pidstat all dead"
fi
