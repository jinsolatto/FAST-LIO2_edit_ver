#!/bin/bash
# bucheon_260914 15degree: 15_d, mid360.yaml only (voxelslam-export
# matching, --rate 1.5, filter_size_surf=0.5), saved to
# bucheon_260914/15degree/15_d/mid360.yaml/voxel0.5/

set -eo pipefail

REPO=/home/jschoi/edit_fastlio2
BAG=/home/jschoi/bag_file/15_d
OUTDIR="$REPO/bucheon_260914/15degree/15_d/mid360.yaml/voxel0.5"
RUNNAME="bucheon_15deg_15_d_mid360_voxel0.5"
...
echo "[15_d/voxel0.5] node ready"

ros2 bag play "$BAG" --rate 1.5 --clock > "/tmp/${RUNNAME}_bagplay.log" 2>&1
echo "[15_d/voxel0.5] bag playback finished"
...
if [ -n "$WIN_ID" ]; then
    ...
    echo "[15_d/voxel0.5] map.png saved (window $WIN_ID)"
else
    echo "[15_d/voxel0.5] WARNING: could not find rviz viewport window for screenshot"
fi

python3 "$REPO/scripts/plot_path_topdown.py" "$OUTDIR/trajectory.tum" "$OUTDIR/path.png"
echo "[15_d/voxel0.5] path.png saved"
...
if [ -n "$LEFTOVER" ]; then
    ...
    echo "[15_d/voxel0.5] confirmed: node/rviz/republish/pidstat all dead"
fi
