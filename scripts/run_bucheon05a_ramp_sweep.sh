#!/bin/bash
# Sweep of bucheon05a.yaml's voxel_size_fine (0.5 -> ramp -> X at 260s, single
# "total" segment, no front/back split) over X in VALUES, against bag
# 05_a_minimize. Saves cpu_log, csv, tum, map.png, path.png per value into
# BUCHEON/bucheon_05a_ramp_260_0X/. Restores bucheon05a.yaml to its current
# state (voxel_size_fine=0.3, ramp_260_03 output paths) when done, regardless
# of success/failure.
...
VALUES=(0.2 0.1)
DIAG=$REPO/diag_test/ramp_sweep
mkdir -p "$DIAG"

cp "$CONFIG" /tmp/bucheon05a_orig_backup.yaml
restore_config() {
    cp /tmp/bucheon05a_orig_backup.yaml "$CONFIG"
    echo "[restore] bucheon05a.yaml restored to voxel_size_fine=0.3 state"
}
...
for v in "${VALUES[@]}"; do
    TAG=$(echo "$v" | tr -d '.')   # 0.2 -> 02, 0.1 -> 01
    OUTDIR="$REPO/BUCHEON/bucheon_05a_ramp_260_${TAG}"
    mkdir -p "$OUTDIR"
...
PYEOF
    ...
    "$REPO/run_with_pidstat.sh" "bucheon05a_ramp_260_${TAG}" bucheon05a.yaml > "/tmp/bucheon05a_ramp_${TAG}_launch.log" 2>&1 &
    LAUNCH_BG_PID=$!

    until grep -q "준비 완료" "/tmp/bucheon05a_ramp_${TAG}_launch.log" 2>/dev/null; do
        sleep 1
    done
    echo "[fine=$v] node ready $(date)"

    ros2 bag play "$BAG" --rate 1.0 --clock > "/tmp/bucheon05a_ramp_${TAG}_bagplay.log" 2>&1
    ...
    cp "$REPO/cpu_logs/cpu_log_bucheon05a_ramp_260_${TAG}.txt" "$OUTDIR/cpu_log.txt"
    echo "=== fine=$v : done $(date) ==="
done

echo "ALL RAMP-FINE-VALUE RUNS COMPLETE $(date)"
