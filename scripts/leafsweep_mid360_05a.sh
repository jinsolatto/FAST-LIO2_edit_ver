...
# against bag 05_a_minimize, saving CSV/TUM/cpu_log/map.png/path.png per value
# into BUCHEON/bucheon_05a_total/<value>/. Restores mid360.yaml to its
...
cp "$CONFIG" /tmp/mid360_orig_backup_05a.yaml

restore_config() {
    cp /tmp/mid360_orig_backup_05a.yaml "$CONFIG"
...
PYEOF
    ...
    "$REPO/run_with_pidstat.sh" "leafsweep05a_${v}" mid360.yaml > "/tmp/leafsweep05a_${v}_launch.log" 2>&1 &
    LAUNCH_BG_PID=$!

    until grep -q "준비 완료" "/tmp/leafsweep05a_${v}_launch.log" 2>/dev/null; do
        sleep 1
    done
    echo "[leaf=$v] node ready"

    ros2 bag play "$BAG" --rate 1.0 --clock > "/tmp/leafsweep05a_${v}_bagplay.log" 2>&1
