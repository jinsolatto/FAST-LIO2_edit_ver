#!/bin/bash
# bucheon_260914 15degree: 15_c, mid360.yaml only (voxelslam-export
# matching, --rate 1.5), saved to bucheon_260914/15degree/15_c/mid360.yaml/

set -eo pipefail

REPO=/home/jschoi/edit_fastlio2
BAG=/home/jschoi/bag_file/15_c
OUTDIR="$REPO/bucheon_260914/15degree/15_c/mid360.yaml"
RUNNAME="bucheon_15deg_15_c_mid360"
...
if [ -n "$WIN_ID" ]; then
    ...
    echo "[15_c] map.png saved (window $WIN_ID)"
else
    echo "[15_c] WARNING: could not find rviz viewport window for screenshot"
fi

python3 "$REPO/scripts/plot_path_topdown.py" "$OUTDIR/trajectory.tum" "$OUTDIR/path.png"
echo "[15_c] path.png saved"
...
if [ -n "$LEFTOVER" ]; then
    ...
    echo "[15_c] confirmed: node/rviz/republish/pidstat all dead"
fi
