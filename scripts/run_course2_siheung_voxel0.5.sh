#!/bin/bash
# One-off: course2_minimize bag, siheung.yaml only (voxelslam-export
# matching, --rate 1.0), saved under
# turtleisland_260915/10degree/course2_minimize/siheung.yaml/voxel0.5/
# (filter_size_surf already 0.5 in siheung.yaml).

set -eo pipefail

REPO=/home/jschoi/edit_fastlio2
BAG=/home/jschoi/bag_file/10degree_/course2_minimize
OUTDIR="$REPO/turtleisland_260915/10degree/course2_minimize/siheung.yaml/voxel0.5"
RUNNAME="course2_siheung_voxel0.5"
...
echo "[course2/siheung.yaml/voxel0.5] node ready"

ros2 bag play "$BAG" --rate 1.0 --clock > "/tmp/${RUNNAME}_bagplay.log" 2>&1
echo "[course2/siheung.yaml/voxel0.5] bag playback finished"
...
if [ -n "$WIN_ID" ]; then
    ...
    echo "[course2/siheung.yaml/voxel0.5] map.png saved (window $WIN_ID)"
else
    echo "[course2/siheung.yaml/voxel0.5] WARNING: could not find rviz viewport window for screenshot"
fi

python3 "$REPO/scripts/plot_path_topdown.py" "$OUTDIR/trajectory.tum" "$OUTDIR/path.png"
echo "[course2/siheung.yaml/voxel0.5] path.png saved"
...
if [ -n "$LEFTOVER" ]; then
    ...
    echo "[course2/siheung.yaml/voxel0.5] confirmed: node/rviz/republish/pidstat all dead"
fi
