...
# against bag 05_d_minimize, saving CSV/TUM/cpu_log/map.png/path.png per value
# into BUCHEON/bucheon_05d_total/<value>/. Restores mid360.yaml to its
...
OUTROOT=$REPO/BUCHEON/bucheon_05d_total
BAG=/home/jschoi/bag_file/05_d_minimize
VALUES=(1.0 0.5 0.4 0.3 0.2 0.1)

cp "$CONFIG" /tmp/mid360_orig_backup_05d.yaml

restore_config() {
    cp /tmp/mid360_orig_backup_05d.yaml "$CONFIG"
...
PYEOF
    ...
    "$REPO/run_with_pidstat.sh" "leafsweep05d_${v}" mid360.yaml > "/tmp/leafsweep05d_${v}_launch.log" 2>&1 &
    LAUNCH_BG_PID=$!

    until grep -q "준비 완료" "/tmp/leafsweep05d_${v}_launch.log" 2>/dev/null; do
        sleep 1
    done
    echo "[leaf=$v] node ready"

    ros2 bag play "$BAG" --rate 1.0 --clock > "/tmp/leafsweep05d_${v}_bagplay.log" 2>&1
    ...
    cp "$REPO/cpu_logs/cpu_log_leafsweep05d_${v}.txt" "$OUTDIR/cpu_log.txt"
    echo "=== leaf=$v : done ==="
done
