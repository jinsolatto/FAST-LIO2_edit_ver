#!/bin/bash
# Sweep of bucheon10b.yaml's voxel_size_fine over VALUES, using the current
# single "total" segment [260, 1000] (ramp from voxel_size_normal=0.3 down to
# voxel_size_fine over voxel_ramp_duration approaching 260s, stays fine after)
# against bag 10_b_minimize. Saves cpu_log, csv, tum, map.png, path.png per
# value into BUCHEON/bucheon_10b/partially/<v>/, matching the path convention
# already set in the file for 0.2. Restores bucheon10b.yaml to its
# pre-sweep on-disk state when done, regardless of success/failure.
...
VALUES=(0.2 0.1)
DIAG=$REPO/diag_test/bucheon10b_sweep
mkdir -p "$DIAG"

cp "$CONFIG" /tmp/bucheon10b_orig_backup.yaml
restore_config() {
    cp /tmp/bucheon10b_orig_backup.yaml "$CONFIG"
    echo "[restore] bucheon10b.yaml restored to its pre-sweep state"
}
...
for v in "${VALUES[@]}"; do
    OUTDIR="$REPO/BUCHEON/bucheon_10b/partially/${v}"
    mkdir -p "$OUTDIR"
...
PYEOF
    ...
    TAG=$(echo "$v" | tr -d '.')
    "$REPO/run_with_pidstat.sh" "bucheon10b_${TAG}" bucheon10b.yaml > "/tmp/bucheon10b_sweep_${TAG}_launch.log" 2>&1 &
    LAUNCH_BG_PID=$!

    until grep -q "준비 완료" "/tmp/bucheon10b_sweep_${TAG}_launch.log" 2>/dev/null; do
        sleep 1
    done
    echo "[fine=$v] node ready $(date)"

    ros2 bag play "$BAG" --rate 1.0 --clock > "/tmp/bucheon10b_sweep_${TAG}_bagplay.log" 2>&1
    ...
    cp "$REPO/cpu_logs/cpu_log_bucheon10b_${TAG}.txt" "$OUTDIR/cpu_log.txt"
    echo "=== fine=$v : done $(date) ==="
done

echo "ALL bucheon10b FINE-VALUE RUNS COMPLETE $(date)"
