#!/bin/bash
# One-off rerun of bucheon05a.yaml as currently configured on disk
# (voxel_size_fine=0.1, output paths already pointed at
# BUCHEON/bucheon_05a/partially/0.1/), overwriting whatever is there.

set -e

REPO=/home/jschoi/edit_fastlio2
CONFIG=$REPO/src/FAST_LIO_ROS2/config/bucheon05a.yaml
OUTDIR=$REPO/BUCHEON/bucheon_05a/partially/0.1
BAG=/home/jschoi/bag_file/05_a_minimize
DIAG=$REPO/diag_test/rerun_01
mkdir -p "$OUTDIR" "$DIAG"
...
echo "=== bucheon05a rerun (fine=0.1) : starting $(date) ==="

"$REPO/run_with_pidstat.sh" "bucheon05a_01_rerun" bucheon05a.yaml > "/tmp/bucheon05a_01_rerun_launch.log" 2>&1 &
LAUNCH_BG_PID=$!

until grep -q "준비 완료" "/tmp/bucheon05a_01_rerun_launch.log" 2>/dev/null; do
    sleep 1
done
echo "[rerun 0.1] node ready $(date)"

ros2 bag play "$BAG" --rate 1.0 --clock > "/tmp/bucheon05a_01_rerun_bagplay.log" 2>&1
echo "[rerun 0.1] bag playback finished $(date)"
...
if [ -n "$WIN_ID" ]; then
    ...
    echo "[rerun 0.1] map.png saved (window $WIN_ID)"
else
    echo "[rerun 0.1] WARNING: could not find rviz viewport window for screenshot"
fi
...
if [ -n "$LEFTOVER" ]; then
    echo "[rerun 0.1] WARNING: leftover processes still alive after cleanup:"
    echo "$LEFTOVER"
else
    echo "[rerun 0.1] confirmed: node/rviz/republish/pidstat all dead"
fi

cp "$REPO/cpu_logs/cpu_log_bucheon05a_01_rerun.txt" "$OUTDIR/cpu_log.txt"
echo "=== bucheon05a rerun (fine=0.1) : done $(date) ==="
