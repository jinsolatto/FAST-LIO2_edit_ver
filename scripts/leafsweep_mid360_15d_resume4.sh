...
# against bag 15_d_minimize, saving CSV/TUM/cpu_log/map.png/path.png per value
# into BUCHEON/bucheon_15d_total/<value>/. Restores mid360.yaml to its
...
OUTROOT=$REPO/BUCHEON/bucheon_15d_total
BAG=/home/jschoi/bag_file/15_d_minimize
VALUES=(0.1)

cp "$CONFIG" /tmp/mid360_orig_backup_15d.yaml

restore_config() {
    cp /tmp/mid360_orig_backup_15d.yaml "$CONFIG"
...
PYEOF
    ...
    "$REPO/run_with_pidstat.sh" "leafsweep15d_${v}" mid360.yaml > "/tmp/leafsweep15d_${v}_launch.log" 2>&1 &
    LAUNCH_BG_PID=$!

    until grep -q "준비 완료" "/tmp/leafsweep15d_${v}_launch.log" 2>/dev/null; do
        sleep 1
    done
    echo "[leaf=$v] node ready"

    ros2 bag play "$BAG" --rate 1.0 --clock > "/tmp/leafsweep15d_${v}_bagplay.log" 2>&1
    ...
    cp "$REPO/cpu_logs/cpu_log_leafsweep15d_${v}.txt" "$OUTDIR/cpu_log.txt"
    echo "=== leaf=$v : done ==="
done
