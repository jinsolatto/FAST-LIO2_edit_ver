...
# against bag 10_b_minimize, saving CSV/TUM/cpu_log/map.png/path.png per value
# into BUCHEON/bucheon_10b_leafsweep_20260901/<value>/. Restores mid360.yaml
# to its original pure state (filter_size_surf 0.5, original output paths)
# when done, regardless of success/failure.

set -e

REPO=/home/jschoi/edit_fastlio2
CONFIG=$REPO/src/FAST_LIO_ROS2/config/mid360.yaml
OUTROOT=$REPO/BUCHEON/bucheon_10b_leafsweep_20260901
BAG=/home/jschoi/bag_file/10_b_minimize
VALUES=(0.5 0.4 0.3 0.2 0.1)  # 1.0 already completed and saved

cp "$CONFIG" /tmp/mid360_orig_backup.yaml

restore_config() {
    cp /tmp/mid360_orig_backup.yaml "$CONFIG"
...
for v in "${VALUES[@]}"; do
    ...
    # edit config: filter_size_surf + output paths for this run
...
PYEOF
    ...
    # launch node + pidstat
    "$REPO/run_with_pidstat.sh" "leafsweep10b_${v}" mid360.yaml > "/tmp/leafsweep10b_${v}_launch.log" 2>&1 &
    LAUNCH_BG_PID=$!

    until grep -q "준비 완료" "/tmp/leafsweep10b_${v}_launch.log" 2>/dev/null; do
        sleep 1
    done
    echo "[leaf=$v] node ready"

    # play bag to completion (blocks)
    ros2 bag play "$BAG" --rate 1.0 --clock > "/tmp/leafsweep10b_${v}_bagplay.log" 2>&1
    echo "[leaf=$v] bag playback finished"

    sleep 2

    # find the rviz 3D viewport window (consistent geometry across launches)
    ...
    cp "$REPO/cpu_logs/cpu_log_leafsweep10b_${v}.txt" "$OUTDIR/cpu_log.txt"
    echo "=== leaf=$v : done ==="
done
