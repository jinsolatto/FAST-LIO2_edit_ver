#!/bin/bash
# suwon_260914 5degree batch: course1 -> course2 -> course3, siheung.yaml
# only (voxelslam-export matching, --rate 1.0), saved into each course's
# siheung.yaml/voxel0.5/ subfolder (existing siheung.yaml/{4 files} left
# untouched). course2_0 bag intentionally skipped per instruction.
#
...
run_course() {
    ...
    local outdir="$OUTROOT/${course}_minimize/siheung.yaml/voxel0.5"
    local runname="suwon_5deg_${course}_siheung_voxel0.5"

    echo "=== $course : starting ==="
    mkdir -p "$outdir"
    set_tum_path "$SIHEUNG_CFG" "$outdir/trajectory.tum"
